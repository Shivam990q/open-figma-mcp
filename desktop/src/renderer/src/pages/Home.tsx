  ...
  return (
    <div className="mx-auto max-w-3xl">
      ...
      <div className="card mt-4 p-5">
        ...
        <ConfigBlock editor={editor} endpoint={endpoint} />

        <div className="mt-3 flex items-start gap-2 text-xs text-faint">
          <Check size={13} className="mt-0.5 shrink-0 text-ok" />
          {editor === 'Lovable'
            ? 'In Lovable: Connectors → Custom MCP → HTTP, then paste the URL above. Start the server first.'
            : `${configHint(editor)}`}
        </div>
      </div>
...
function configHint(editor: Editor): string {
  switch (editor) {
    case 'Cursor':
      return 'Cursor: Settings → MCP → Add new server, or create .cursor/mcp.json with the above.'
    case 'VS Code':
      return 'VS Code: create .vscode/mcp.json (or use the MCP: Add Server command) with the above.'
    case 'Claude':
      return 'Claude Desktop: edit claude_desktop_config.json with the above, then restart Claude.'
    case 'Windsurf':
      return 'Windsurf: Settings → Cascade → MCP servers → add the above.'
    default:
      return ''
  }
}

function ConfigBlock({ editor, endpoint }: { editor: Editor; endpoint: string }) {
  const config =
    editor === 'Lovable'
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-border bg-bg">
      <div className="flex items-center justify-between border-b border-border px-3 py-1.5">
        <span className="text-xs text-faint">{editor === 'Lovable' ? 'HTTP (Streamable)' : 'stdio'}</span>
